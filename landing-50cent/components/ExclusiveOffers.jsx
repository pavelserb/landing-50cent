...
export default function ExclusiveOffers({ title, description, buttonText, link }) {
    ...
    return (
      <div className="p-6 bg-white rounded-lg shadow-lg flex flex-col justify-between">
        <div>
          <h3 className="text-xl font-heading mb-2 font-bold">{title}</h3>
          <p className="text-gray-600">{description}</p>
        </div>
        <a
          href={link}
          className="mt-4 inline-block bg-accent text-black font-semibold py-2 px-4 rounded hover:opacity-90 transition"
        >
          {buttonText}
        </a>
      </div>
